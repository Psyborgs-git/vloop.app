...
import { LlmAgent, FunctionTool, InMemoryRunner, LLMRegistry } from '@google/adk';
...
import type { AgentConfigId, ChatSessionId, WorkflowId, ModelId, ModelConfig, ProviderConfig } from './config/types.js';
...
export class AgentOrchestrator {
    ...
    constructor(
        ...
        this.agentBuilder = new AgentBuilder(configStore!, this.providerRegistry, tools, logger);
        this.workflowRunner = new WorkflowRunner(configStore!, this.agentBuilder, logger);
    ...
    /**
     * Legacy workflow — direct prompt with model string.
     * Uses ADK LlmAgent with the registered orchestrator tools.
     */
    public async runLegacyWorkflow(
        opts: LegacyWorkflowOptions,
        emit?: (type: 'stream' | 'event', payload: unknown, seq?: number) => void,
        context?: HandlerContext,
    ): Promise<any> {
        this.logger.info({ workspaceId: opts.workspaceId, model: opts.model }, 'Starting legacy LLM workflow');

        const model = opts.model || 'gemini-2.5-flash';

        // Map Orchestrator ToolRegistry into ADK FunctionTools
        ...
        const agent = new LlmAgent({
            ...
            model,
            description: 'Orchestrator AI Agent with root daemon access.',
            instruction: 'You are the Orchestrator AI Agent. You have root access to a powerful server daemon that can manage containers, spawn processes, and control web browsers. Your job is to fulfill the user requests completely by chaining together your available tools. Only report your final conclusions after using the tools. Plan out your execution.',
            tools: adkTools,
        });

        const runner = new InMemoryRunner({
            agent,
            appName: 'orchestrator_legacy',
        });
    ...
    public async runAgentChat(
        ...
        this.logger.info({ agentId: opts.agentId, sessionId: opts.sessionId }, 'Starting agent chat');

        // Persist user message
        const agentConfig = this.configStore.getAgent(opts.agentId);
        if (!agentConfig) throw new Error(`Agent config not found: ${opts.agentId}`);

        const resolvedRuntime = await this.providerRegistry.resolve(agentConfig.modelId, this.vaultGet, agentConfig.params);
        ...
        const built = await this.agentBuilder.build(opts.agentId, this.vaultGet, context);
        ...
        for await (const event of events) {
            ...
            if (emit) {
                ...
                if (chunkText) mappedEvent.text = chunkText;
                if (chunkToolCalls.length > 0) mappedEvent.toolCalls = chunkToolCalls;
                if (chunkToolResults.length > 0) mappedEvent.toolResult = chunkToolResults[0]; // ChatView expects single toolResult
                emit('stream', mappedEvent, seq++);
            }
        }
        ...
        this.knowledgeGraph.indexText(opts.agentId, opts.sessionId, `${opts.prompt}\n${fullText}`);
    ...
    public async runWorkflow(
        ...
        return this.workflowRunner.run(workflowId, input, emit, this.vaultGet, context);
    }
    ...
    public async runChatCompletion(
        opts: { model?: string; modelId?: ModelId | string; prompt: string; systemPrompt?: string; sessionId?: ChatSessionId },
        emit?: (type: 'stream' | 'event', payload: unknown, seq?: number) => void,
    ): Promise<any> {
        const resolved = await this.resolveRuntimeForCompletion(opts);
        this.logger.info({ model: resolved.model.modelId, adapter: resolved.adapter }, 'Starting chat completion');
        ...
        const agent = new LlmAgent({
            ...
            description: 'Simple LLM chat assistant.',
            instruction: opts.systemPrompt || 'You are a helpful AI assistant. Respond clearly and concisely.',
            tools: [],
        ...
        this.knowledgeGraph.indexText(undefined, opts.sessionId, `${opts.prompt}\n${fullText}`);

        return { status: 'completed', model: resolved.model.modelId, result: fullText };
    }

    private async resolveRuntimeForCompletion(opts: { model?: string; modelId?: ModelId | string }): Promise<ResolvedModel> {
        ...
        if (ref && this.configStore) {
            ...
            const byModelString = this.configStore.listModels().find(m => m.modelId === ref);
            if (byModelString) {
